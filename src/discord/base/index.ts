...
export async function bootstrap(opts: BootstrapOptions) {
  ...
  const partials = (rawPartials as any[]).map((p) => {
    if (typeof p === "string") {
      switch (p) {
  ...
  client.once("ready", async () => {
    if (typeof opts.whenReady === "function") {
      await opts.whenReady(client as MaybeClient);
    }
  });

  return client as MaybeClient;
}
