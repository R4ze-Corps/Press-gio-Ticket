...
export default localCreateResponder({
    ...
    async run(interaction) {
        try {
            ...
            const buttonsRow = new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId("farm-registrar").setLabel("🚜 Registrar Farm").setStyle(ButtonStyle.Primary), new ButtonBuilder().setCustomId(`farm-ver-entregas_${userId}`).setLabel("📊 Ver Entregas").setStyle(ButtonStyle.Success), new ButtonBuilder().setCustomId(`farm-finalizar_${userId}`).setLabel("💽 Finalizar").setStyle(ButtonStyle.Secondary), new ButtonBuilder().setCustomId(`farm-fechar_${userId}`).setLabel("🔒 Fechar Canal").setStyle(ButtonStyle.Danger));
            const container = createContainer("#2b2d31", welcomeContent, buttonsRow);
